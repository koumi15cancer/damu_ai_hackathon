...
def test_analytics_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Total events analyzed: {data['analytics_summary']['total_events']}")
            print(f"   Suggestions generated: {len(data['suggestions'])}")
            print(f"   Most popular theme: {data['analytics_summary']['most_popular_theme']}")
            print(f"   Average cost: {data['analytics_summary']['average_cost']:,.0f} VND")
        ...
        for limit in [5, 10, 20]:
            response = requests.get(f"http://localhost:5000/analytics/suggestions?limit={limit}")
            if response.status_code == 200:
                data = response.json()
                print(f"   ✅ Limit {limit}: {data['analytics_summary']['total_events']} events analyzed")
            else:
        ...
        for theme in themes:
            response = requests.get(f"http://localhost:5000/analytics/suggestions?theme={theme}")
            if response.status_code == 200:
                data = response.json()
                print(f"   ✅ Theme '{theme}': {data['analytics_summary']['total_events']} events")
            else:
        ...
        try:
            ...
            print("   ⚠️  Analytics data file not found")
